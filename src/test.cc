...
/* testcase comes from https://github.com/MichaelSuen-thePointer/SimpleJSON/blob/master/SimpleJSON/main.cpp */
...
AUTO_TEST_CASE(json_edit_test)
{
	...
	*/
    ...
    assert((doc[111] == nullptr)); //new initialized value is null
...
AUTO_TEST_CASE(json_string_escape_sequence_test)
{
    ...
    auto _2 = Json::parse(R"("\" \\ \/ \b \f \n \r \t")", err);
    assert((_2.as_string() == "\" \\ / \b \f \n \r \t"));
    assert((err == ""));

    //auto _3 = Json::parse(R"("\u0024 \u20AC \uD801\uDC37 \uD852\uDF62")");
    auto _3 = Json::parse(R"("\u0024 \u20AC \uD801\uDC37 \uD852\uDF62")");
    // assert((_3.as_string() == "\x24 \xe2\x82\xac \xf0\x90\x90\xb7 \xf0\xa4\xad\xa2"));
    assert((err == ""));
}
...
{
    auto j = Json::parse("100000000000000000000000000"); //1e26 fallsback to double
    // assert((j == 1e26));
}
