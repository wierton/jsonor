...
public:
  ...
  virtual bool to_boolean() const {
    throw std::runtime_error(
        "to_boolean however not an boolean");
  }
  virtual int64_t to_integer() const {
    throw std::runtime_error(
        "to_integer however not an integer");
  }
  virtual double to_float() const {
    throw std::runtime_error(
        "to_float however not an float");
  }
  virtual std::string to_string() const {
    throw std::runtime_error(
        "to_string however not an string");
  }

  virtual std::string &as_string() {
    throw std::runtime_error(
        "as_string however not an string");
  }
  virtual Json::Array &as_array() {
    throw std::runtime_error(
        "as_array however not an array");
  }
  virtual Json::Object &as_object() {
    throw std::runtime_error(
        "as_object however not an object");
  }
  ...
  virtual size_t size() const {
    throw std::runtime_error(
        "fetch size however not an array or object");
  }
  virtual Json &at(const std::string &) {
    throw std::runtime_error(
        "fetch element with key however not an object");
  }
  virtual Json &at(const size_t idx) {
    throw std::runtime_error("fetch element with subscript "
                             "however not an array");
  }
...
public:
  ...
  std::string &as_string() override { return s; }
...
public:
  ...
  JArray(std::vector<Json> &&array)
      : array(std::move(array)) {}
  JArray(std::initializer_list<Json> &&ilist)
      : array(ilist) {}
  Json::Type type() const override { return Json::ARRAY; }
  size_t size() const override { return array.size(); }

  Json::Array &as_array() override { return array; }

  const Json::Array &as_array() const override {
    return array;
  }
...
public:
  ...
  JObject(Json::Object &&object)
      : object(std::move(object)) {}
  Json::Type type() const override { return Json::OBJECT; }

  Json::Object &as_object() override { return object; }

  const Json::Object &as_object() const override {
    return object;
  }
...
// Json
Json::Json() : je(new JNull()) {}

Json::Json(nullptr_t) : je(new JNull()) {}

Json::Json(bool b) : je(new JBoolean(b)) {}

Json::Json(int i) : je(new JInteger(i)) {}

Json::Json(int64_t i) : je(new JInteger(i)) {}

Json::Json(double f) : je(new JFloat(f)) {}

Json::Json(const char *s) : je(new JString(s)) {}

Json::Json(const std::string &s) : je(new JString(s)) {}

Json::Json(std::string &&s)
    : je(new JString(std::move(s))) {}

Json::Json(const Array &array) : je(new JArray(array)) {}

Json::Json(Array &&array)
    : je(new JArray(std::move(array))) {}

Json::Json(const Object &obj) : je(new JObject(obj)) {}
...
Json::Json(Json &&other) : je(other.je) {}
...
int64_t Json::to_integer() const {
  return je->to_integer();
}
...
Json JParser::MatchPrimary(std::istream &is) {
  ...
  default:
    throw std::runtime_error("unexpected character meet "
                             "whilc process primary "
                             "token " +
                             std::to_string(ch));
  }
}
...
Json JParser::MatchArray(std::istream &is) {
  ...
  while (!isEof(is)) {
    ...
    if (ch == ']')
      ...
      throw std::runtime_error("expected ',' or ']' while "
                               "processing array, not " +
                               ch);
  }
...
Json JParser::MatchObject(std::istream &is) {
  ...
  while (!isEof(is)) {
    ...
    object.insert(std::make_pair<std::string, Json>(
